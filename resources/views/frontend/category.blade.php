            ...
            <div class="col-sm-3">
               <div class="shop-sidebar">
                  ...
                  @if ($slug=='monitor')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterMntRes',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->mnt_resolution}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">Response Time</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterMntTime',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->mnt_response_time}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">Screen Size</h5>
                     <ul class="shop-cate">
                        @foreach($record[2] as $rec2)
                           <li><a href="{{action('FrontendController@getFilterMntSize',['slug'=>$slug,'filter'=>$rec2->id])}}">{{$rec2->mnt_screen_size}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">Refresh Rate</h5>
                     <ul class="shop-cate">
                        @foreach($record[3] as $rec3)
                           <li><a href="{{action('FrontendController@getFilterMntRate',['slug'=>$slug,'filter'=>$rec3->id])}}">{{$rec3->mnt_refresh_rate}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='cpu')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterCpuSeries',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->cpuserie}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">Cpu Socket</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterCpuSk',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->socket_type}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='case')
                     ...
                     <ul class="shop-cate">
                        @foreach($record as $rec0)
                           <li><a href="{{action('FrontendController@getFilterCase',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->case_type}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='hdd')
                     ...
                     <ul class="shop-cate">
                        @foreach($record as $rec0)
                           <li><a href="{{action('FrontendController@getFilterHDD',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->drive_capacity}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='vga')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterVgaGpu',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->vga_gpu}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">Memory</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterVgaMem',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->vga_mem_size}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='ssd')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterSsdFF',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->ssd_form_factor}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">Interface</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterSsdIF',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->ssd_interface}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='ram')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterRamCa',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->ram_capacity}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">speed</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterRamSp',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->ram_speed}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='psu')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterPsuEE',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->psu_ee}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">power</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterPsuPW',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->psu_power}}</a></li>
                        @endforeach
                     </ul>
                  @elseif ($slug=='mainboard')
                     ...
                     <ul class="shop-cate">
                        @foreach($record[0] as $rec0)
                           <li><a href="{{action('FrontendController@getFilterMbChip',['slug'=>$slug,'filter'=>$rec0->id])}}">{{$rec0->mb_chipset}}</a></li>
                        @endforeach
                     </ul>
                     <h5 class="shop-tittle margin-bottom-30">size</h5>
                     <ul class="shop-cate">
                        @foreach($record[1] as $rec1)
                           <li><a href="{{action('FrontendController@getFilterMbSize',['slug'=>$slug,'filter'=>$rec1->id])}}">{{$rec1->mb_size}}</a></li>
                        @endforeach
                     </ul>
                  @endif
